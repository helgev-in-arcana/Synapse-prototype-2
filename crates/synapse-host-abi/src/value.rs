...
//!
//! # 意図
//! ABI 上の値は [`SynValue`] による**値渡し**（ADR-022）で、≤ポインタ幅の小さな値は
//! `ptr` フィールドへインライン格納する SVO（Small Value Optimization）、それを超える大型値は
//! 別領域を指す。本モジュールの [`OwnedValue`] は、この二様式を**ホスト所有の連続バイト列**へ
//! 正規化して隠す。プラグインへ渡すとき／受け取るときの境界変換をここに閉じ込める。
//!
//! # 使い方
...
use synapse_abi::{SynTypeId, SynValue, SYN_URID_INVALID};

use crate::ffi::PTR_SIZE;

/// ホスト所有の値。
///
/// SVO（≤ポインタ幅）でも大型でも、内部表現は統一して「先頭から `size` バイトに実体を持つ
/// バイト列」とする。`bytes` の確保長は `size.max(PTR_SIZE)`（SVO 経路で `to_value` が
/// ポインタ幅ぶん読み出せるようにするためのパディング）。
#[derive(Clone)]
pub struct OwnedValue {
    pub(crate) type_id: SynTypeId,
    pub(crate) size: usize,
    pub(crate) bytes: Vec<u8>, // 長さは size.max(PTR_SIZE)
}
...
impl OwnedValue {
    ...
    ///
    /// # Safety
    /// `v` は妥当な `SynValue`。大型（>ptr 幅）の場合 `v.ptr` は `v.size` バイト読める領域を
    /// 指していなければならない。
    ...
    ///
    /// ≤ポインタ幅は SVO（`ptr` フィールドへインライン）、大型は `self.bytes` を指す。後者は
    /// `self` が生存する間のみ有効。
    ///
    /// # Safety
