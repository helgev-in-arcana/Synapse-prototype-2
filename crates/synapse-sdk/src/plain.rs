...
//! （≤ポインタ幅はインライン、>ポインタ幅はホスト確保バッファ経由）の差を変換関数に閉じ込める。
//!
//! # 安全性の要
...
///
/// ≤ポインタ幅は SVO（`SynValue::ptr` へインライン）、>ポインタ幅はホスト確保バッファ経由で
/// 値渡しする。
///
/// # Safety
...
/// プラットフォームのポインタ幅（バイト）。SVO 経路の判定境界。
pub(crate) const PTR_SIZE: usize = core::mem::size_of::<*mut c_void>();

/// ≤ポインタ幅の値を `SynValue`（SVO: 値を `ptr` フィールドにインライン格納）へ組み立てる。
///
/// 出力・既定値の「≤ptr 経路」で共用する。`v` は呼び出し側に生きている参照（コピーするだけ）。
///
/// # Safety
/// `T: SynPlainType` で `T::SIZE == size_of::<T>()`（`_SIZE_CHECK` で強制）。`T::SIZE` は
/// ポインタ幅以下でなければならない（呼び出し側が分岐で保証する）。
...
/// `SynValue` を Rust 値へ変換する。サイズ不一致は `None`（>ptr 異型の範囲外読みを遮断）。
///
/// `T::SIZE <= ptr` か否かで SVO（`ptr` フィールド）/ 領域（`*ptr`）を出し分ける（32-bit でも
/// 正しい）。型 ID の照合は呼び出し側（[`crate::context::ProcessCtx`] がポートの type_id と
/// 突合）が行う。
///
/// # Safety
/// `v` は妥当な `SynValue`。大型（>ptr）のとき `v.ptr` は `T::SIZE` バイト読める領域を指す。
